...
import type { Client, Treatment } from '@/types/clinic';
...
export default function ClientTreatmentsCreate({ client, treatments, combos, discountRules }: Props) {
    ...
    const parsedDiscount = isCourtesy ? 0 : parseFloat(discountPercent) || 0;
    const calculatedTotal = preview?.calculated_price ?? 0;
    const finalTotal = isCourtesy ? 0 : preview ? applyPurchaseDiscount(calculatedTotal, parsedDiscount) : 0;
    ...
    const toggleCourtesy = (checked: boolean) => {
        ...
        if (checked) {
            ...
            setPayments([createEmptyPurchasePayment()]);
        }
    };
    ...
    const fetchPreview = useCallback(async () => {
        ...
        const response = await fetch(routes.clientTreatments.preview(), {
            ...
            body: JSON.stringify({
                ...
                payments: isCourtesy
                    ? []
                    : validPaymentsForPreview(payments).map((payment) => ({
        ...
        setPreview(data);
        setPreviewError(null);
    }, [billingMode, isCourtesy, parsedDiscount, payments, selectedIds]);
    ...
    const canSubmit = isCourtesy
        ? selectedIds.length > 0 && treatments.length > 0 && preview !== null
    ...
    return (
        <>
            <Head title={`Tratamentos — ${client.name}`} />
            <div className="page-container mx-auto w-full max-w-2xl">
                <PageHeader
                    title="Atribuir tratamentos"
                    description={`Cliente: ${client.name}`}
                />

                <Form
                    action={`/clients/${client.id}/treatments`}
                    method="post"
                    className="space-y-6"
                >
                    {({ processing, errors }) => (
                        <>
                            ...
                            <div className="grid gap-2">
                                ...
                                <TreatmentCatalogSelect
                                    ...
                                    emptyMessage="Cadastre tratamentos em Tratamentos antes de atribuir ao cliente."
                                    includeHiddenInputs
                                ...
                                {isCombo && combo && (
                                    <p className="text-muted-foreground text-sm">
                                        ...
                                        tratamento): o mais caro sem desconto adicional; os demais
                                        com {parseFloat(combo.extra_discount_percent)}% de desconto.
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="grid gap-2">
                                <label className="flex items-center gap-2">
                                    <input
                                        type="checkbox"
                                        checked={isCourtesy}
                                        onChange={(event) => toggleCourtesy(event.target.checked)}
                                    />
                                    Tratamento de cortesia
                                </label>
                                <input type="hidden" name="is_courtesy" value={isCourtesy ? '1' : '0'} />
                                <p className="text-muted-foreground text-xs">
                                    Cortesia atribui as sessões sem cobrança (R$ 0,00).
                                </p>
                                <InputError message={errors.is_courtesy} />
                            </div>
                            ...
                            {preview && (
                                <PurchaseDiscountValidation
                                    ...
                                    includeFormFields
                                    hidePricing={isCourtesy}
                                    discountError={errors.discount_percent}
